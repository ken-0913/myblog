...
2. **그 등급 안에서 테넌트를 고른다.** 이 글에서는 번갈아 가며 한 명씩(round-robin) 돌린다.
...
**EPP 기본값은 테넌트 구분을 무시하는 `global-strict` + FCFS다.** 기존 동작을 그대로 흉내내기 위한 선택인데, 그러면 공정성 실험이 되지 않는다. 그래서 이 글은 가이드 권장대로 fairness만 **round-robin**으로 바꿔 진행했다.
...
**4건의 premium이 모두 standard보다 먼저 나갔고, 둘 다 큐에 있던 best-effort 전부를 제쳤다.** 등급을 고르는 1단계가 하드코딩된 엄격한 순서라는 것이 그대로 관찰된다.
