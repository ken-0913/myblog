...
사용률이 낮은데 부하가 걸려 있으면 배칭 문제를, 전력은 높은데 처리량이 낮으면 메모리 병목을 의심한다. 이 기준으로 이후 벤치마크 결과의 원인을 구분한다.
...
vLLM을 기동하고 벤치마크 트래픽을 준비한다. 프리필과 디코드는 서로 다른 최적화 전략을 요구하므로, 실제 사용 패턴과 닮은 벤치마크 데이터가 최우선이다.

**상호보완적인 데이터셋 두 가지**를 사용한다.
...
```bash
nohup vllm serve Qwen/Qwen3-14B > vllm.log 2>&1 &
tail -f vllm.log
```



정상적으로 뜨면 아래와 같은 로그가 찍힌다.
...
`/workspace/vllm` 이하에 `inspect_dataset.py`를 생성한다.

<details class="orca-details">
<summary>전체 코드 보기 (inspect_dataset.py)</summary>
...
별도 튜닝 없이 기본 설정으로 `vllm serve Qwen/Qwen3-14B`를 실행하고, 로그를 확인해 베이스라인을 정한다.

- 모델 가중치 **27.5GB** + KV 캐시 **11GB**(72,064토큰) = 총 38.5GB/46GB 사용
- **모델이 GPU 메모리의 65% 이상 차지** → KV 캐시 공간 부족 → 배치 축소·캐시 축출 증가 → 재계산 증가 → 처리량 저하라는 인과 체인(causal chain)이 성립한다. 이것이 이후 양자화의 필요성을 뒷받침한다. (모델 자체를 작게 만드는 것의 중요한 효과 중 하나는 남는 VRAM을 KV 캐시에 돌릴 수 있다는 점이다)
...
즉 **캐싱이 처리량은 확실히 올리지만, 동시성이 함께 늘면 TTFT는 오히려 나빠질 수 있다.** 책에는 없지만 이번 실습에서 확인된 관찰이다.
...
세밀하게 튜닝한 설정 파라미터를 적용해 vLLM 서버를 재기동한다.
