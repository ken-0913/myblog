...
Qwen2.5-72B(bf16 약 144GB)는 반대로 TP=4가 TP=2×DP=2보다 처리량(`3.45` vs `3.20 req/s`)과 TPOT(`72.04` vs `84.62ms`)에서 앞섰고, DP=4는 당연 단일 GPU의 VRAM부족으로 시도조차 불가능했다. **모델이 GPU 한 장에 들어가는지 여부가 TP/DP 비중을 정하는 1차 기준**일 것이다.
